...
- **In progress (Step 3):** six scrapers integrated — Isthmus (iCal + RSS), Visit Madison (Simpleview JSON API), High Noon Saloon (HTML calendar), Our Lives (Tribe Events REST), Ticketmaster (Discovery API, multi-venue: Sylvee, Majestic, Orpheum, Overture, Kohl Center, etc.), and Atwood Music Hall (Squarespace events collection — also surfaces sister-venue shows at Barrymore Theatre + Liquid). All but High Noon and Atwood use a 30-day forward window; those two pull whatever the venue has posted (typically several months). More candidate sources in `docs/EVENT_SOURCES.md`; daily scheduling not yet wired up (no APScheduler — recommend running `/admin/scrape` from cron / systemd timer / external scheduler).
