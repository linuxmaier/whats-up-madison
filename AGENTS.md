...
- **A cached miss is permanent, including one caused by a transient failure.** `_call_nominatim` returns `"error"` for *any* exception — rate limiting and 5xx included — and `geocode_lookup` caches that and never retries it. A burst of fresh keys can therefore poison the cache wholesale: after the #236 key-format change, the first production scrape had to resolve hundreds of new keys at 1 req/sec and a large share cached as failures, leaving 31.6% of events without coordinates even though their addresses resolved fine when queried by hand. If coverage drops after any change touching `normalize_lookup`, suspect this first and clear it with `?force=true`. Tracked in #253.
