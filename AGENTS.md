...
- **Source-priority merge** — when a later source has higher trust rank (per `SOURCE_PRIORITY` in `ingest.py`, mirroring the frontend list), it overwrites all non-null `_OVERWRITABLE_FIELDS` (title, description, end_at, venue_name, venue_address); equal- or lower-priority sources only fill null fields
