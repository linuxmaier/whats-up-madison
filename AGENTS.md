...
- **Fuzzy dedup** — after an exact hash miss, a secondary search matches candidates by time+venue and scores title similarity via `difflib.SequenceMatcher`; events scoring ≥ `FUZZY_TITLE_THRESHOLD` (0.65) are treated as duplicates and merged rather than inserted as new rows; this catches near-identical events listed under slightly different titles by different sources. The time anchor is SQL-side (exact `start_at`, or the date for all-day events); the venue anchor is applied in Python via `canonical_venues.venues_match` because it can't be expressed as SQL equality (see *Venue name matching*)
  ...
  **A shared headliner promotes a low score to the threshold (#243).** Sources routinely agree on the act and disagree on everything after it: Isthmus lists the full bill (`Max McNown, Sam Burchfield`) where Ticketmaster lists the tour (`Max McNown - The Summer Vacation Tour`). That pair scores **0.51** — real duplicates like it sit at 0.32–0.52, well below any threshold that would be safe to set. `_headliner_match` strips a status prefix (`SOLD OUT:`, `CANCELED:`), splits on the first separator (spaced dash, `,`, `:`, `+`, `|`, ` with `, ` feat`), and compares the leading segments by equality or word-boundary prefix containment, with a 5-character floor so a very short leading token can't carry a merge alone. The unspaced-hyphen distinction matters — `Teeny-Tiny Terrace Trot` must not split.

  Three properties make this safe: it runs **after** `_shares_significant_token`, so it can't smuggle a pair past the #246 guard; it only ever *raises* a score to exactly `FUZZY_TITLE_THRESHOLD`, so it adds merges and never removes one; and the threshold itself is untouched, so a genuinely high-scoring candidate still outranks a headliner match. Measured against 96 anchor-sharing pairs in a fresh production snapshot: **9 pairs newly merged (all genuine duplicates), 0 existing merges broken**, and the whole #236/#246 negative set stays separate. The debug log prints `Fuzzy match (headliner)` rather than a score when this path fires.

  **A venue anchor is mandatory, not optional (#258).** `_find_fuzzy_duplicate` used to only apply the venue filter when the incoming event *had* a venue; with none, every candidate sharing the time anchor was judged on title similarity alone. That produced a real wrong merge: `Waunakee Farmers' Market` and `Monroe Farmers' Market` — two different markets in two different towns — cleared the #246 significant-token guard on the shared words "farmers"/"market" and scored 0.78, with nothing else to catch the mismatch. A production sweep (grouping all active events by anchor exactly as `reconcile_duplicate_events` does, and re-running `_find_fuzzy_duplicate` against them) found **10 more pairs like it** sitting in production as still-separate rows, one match away from being wrongly merged: two more Waunakee/Monroe recurrences, `Capitol View Farmers' Market` ↔ `Verona Farmers' Market` (×3), `Shawano Folk Festival` ↔ `White Oak Folk Fest`, `Art on Main` ↔ `Third Ward Moon Festival`, and two `Volunteer with X` civic listings promoted to threshold by the #243 headliner rule on the shared prefix "volunteer with" (a rule meant for touring acts, which always have a venue). Against those 10, exactly **1** pair looked like a legitimate near-duplicate: an `AtwoodFest` listing matching its own `CANCELED:`-prefixed re-post, both venue-less. `_find_fuzzy_duplicate` and `_fuzzy_find_event` now both refuse to fuzzy-match at all when the incoming event has no venue anchor — a 10:1 margin, and consistent with #246's principle that a wrong merge (invisible, hides a real event) is worse than a duplicate (visible). Don't reintroduce a no-venue fallback without re-measuring against fresh production data first.
...
#### Reconciling pre-existing duplicates (`reconcile_duplicate_events`, #245)

Fuzzy dedup above only runs at insert time, when a freshly-scraped raw's `canonical_hash` misses against the DB. Once two `Event` rows exist for the same real-world event, every later scrape of either source recomputes the *same* hash it already produced and hits its own row directly — fuzzy matching, and any later fix to it, is never consulted again for that pair. The normal staleness self-heal in `finalize_source_run` can't rescue this either, since it only fires when a row's *last* active source goes quiet; a duplicate with a distinct active source on each side is permanent.

This is real, not hypothetical: #245 found the issue's own example live in production — two Overture Center "Great Gatsby" rows created 8 minutes apart a month before the venue-matching fix (#236) that would have caught them, one kept alive indefinitely by Ticketmaster re-confirming it daily.

`reconcile_duplicate_events(db, *, dry_run=True)` in `ingest.py` is a manual sweep over the whole active table that catches this. It groups `Event` rows by the same anchor `_fuzzy_find_event` uses (exact `start_at`, or date for all-day events), then within each group walks events in `created_at` order matching each one against representatives seen so far — via `_find_fuzzy_duplicate`, the same Python-side venue+title matching logic `_fuzzy_find_event` uses (extracted out so the two paths can't drift apart). For each match:

- **Survivor** is the side with the higher-trust active source (`_best_existing_rank`, same ranking `ingest_chunk` uses), tie-broken by earlier `created_at`.
- **Fields** null-fill from the loser into the survivor (one-directional — the survivor is already the higher-priority side, so there's no overwrite branch). Categories are unioned.
- **EventSource rows** move to the survivor; if both sides already carry a row for the same `source_name` (the Great Gatsby shape — both rows had an Isthmus link), the fresher/active one wins and the other is dropped, rather than violating the `(event_id, source_name)` unique constraint.
- **The loser is soft-removed** (`status = "removed"`, sources emptied), not hard-deleted — consistent with events never being hard-deleted (see *Event Status* below). It becomes an inert row no future pass will touch.

Exposed as `POST /admin/reconcile-duplicates?dry_run=true|false`, gated by `require_admin_key` and a `threading.Lock` (409 on overlap) like `/admin/geocode`. Defaults to `dry_run=true` — merging is less reversible than the additive/corrective work `/admin/geocode` and `/admin/tag` do, so the first call previews the merges it would make (returned in the response `details` list) without writing; pass `?dry_run=false` to commit. Deliberately **not** wired into `/admin/scrape` — it's an O(events² within anchor) sweep over the whole active table, and the drift it cleans up accumulates slowly, not every run.
...
- **Resolution order is canonical registry → address key → venue-name key (#247).** The address stays preferred because a street address is more precise; the name is only tried when the address key resolved to nothing. Some sources ship an address OSM has no node for while carrying the venue under its name — Isthmus's `5950 golf course road, spring green` misses where `American Players Theatre, Spring Green` resolves. Both keys cache independently, so the fallback costs one extra request per venue, not per event, and it can only ever *add* coordinates.
- Changing the lookup-key format orphans existing `venue_geocodes` rows (they're keyed by the old string). The old rows are never consulted again, but the *new* keys all have to be resolved from scratch — see the caching warning below before assuming this is free.
- **Failed lookups are cached, but `error` rows expire (#253).** `not_found` and `success` are answers Nominatim actually gave, so they're kept indefinitely; `?force=true` is the way to re-check them. An `error` row is different — `_call_nominatim` reports `"error"` for *any* exception, rate limiting and 5xx included, so it usually means "we couldn't ask right now" rather than "this place doesn't exist". Those are re-attempted once older than `_ERROR_RETRY_AFTER` (6 hours), and the retry **updates the row in place** — bumping `attempts`, refreshing `geocoded_at` — rather than inserting a second row for the same key.

  Why this matters: treating transient failures as permanent poisoned the cache wholesale after #236 changed the key format. Every cached row was orphaned, hundreds of fresh keys had to resolve back-to-back at 1 req/sec, a large share failed transiently, and production sat at **31.6% of events without coordinates** — while 11 of the top 15 uncoordinated venues resolved fine when queried by hand. A `?force=true` pass cleared 173 such rows and recovered 166 events immediately. A TTL rather than not caching errors at all: during a real Nominatim outage, not caching would make every event re-hit the network on every pass.

- **The cache survives concurrent writers (#255).** `geocode_lookup` does check-then-insert, which is not atomic; `?force=true` deleting rows underneath a concurrent run makes the window easy to hit. A duplicate insert is caught, rolled back, and the winner's row is read back instead of propagating. This used to be fatal — one `UniqueViolation` left the session rolled back, so every later query in `geocode_all_missing` raised, the loop counted each as an error, and the rest of the pass silently did nothing. Both runners now `db.rollback()` in their exception handler so one bad event can't void the others.

- **`/admin/geocode` refuses overlapping runs with 409** and returns **500** when a pass raises. It previously returned 200 with an `{"error": ...}` body, which made an aborted backfill indistinguishable from a successful one — worth remembering when reading old run output.
...
`ADMIN_API_KEY` gates the `/admin/scrape`, `/admin/tag`, `/admin/geocode`, and `/admin/reconcile-duplicates` endpoints. All four require an `X-Admin-Key: <key>` request header. In development (`ENVIRONMENT=development`) with no key set the check is bypassed so existing dev workflows keep working. In production the app refuses to start if `ADMIN_API_KEY` is unset.
...
To find and merge pre-existing duplicate rows (see *Reconciling pre-existing duplicates* above): `curl -X POST 'http://localhost:8000/admin/reconcile-duplicates'` previews the merges it would make; add `?dry_run=false` to actually commit them.
