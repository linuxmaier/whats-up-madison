...
- URL: https://downtownmadison.org/calendar/list/
- Scraper type: api (retired) — a future integration would be html
- Status: **deferred**
- Reason: Integrated for #72 in May 2026, retired in #252 after the REST integration broke in three independent ways. (1) **`downtownmadison.org` strips query strings site-wide.** Any request with a query redirects `https://…?params` → 301 → `http://…` (query gone, protocol downgraded) → 301 → `https://…`. `httpx` doesn't follow redirects by default, so the scrape raised outright; following them yields an unfiltered, unpaginated response. This is not limited to `/wp-json/` — `/calendar/?eventDisplay=list&tribe_paged=2` loses its query too. Every param the scraper relied on (`categories`, `start_date`, `end_date`, `per_page`, `page`) was therefore inert. (2) **REST pagination is unusable.** The API's own `next_rest_url` is query-based and gets stripped identically, so every page returns the same first 12 of 25 events — verified by walking it six times. (3) **The `dmi-events` category no longer exists.** The taxonomy is now per-series (`whats-up-downtown`, `new-faces-new-places`, `new-member-coffee`, `annual-celebration`, `board-of-directors`, `executive-committee`, `transportation`, `economic-development`, `quality-of-life`, `government-relations`, `beyond-compliance`, `inclusiveness`), so the filter would match nothing even with a working query string. The site's WAF also 403s some clients (`urllib`'s default UA) while allowing others, so any future integration needs a descriptive browser UA like the City of Madison scraper.
- Revival path (verified working, 2026-07-25): `GET /calendar/list/page/N/` uses **path-based** pagination, needs no query string, and serves a JSON-LD `Event` array (`name`, `startDate`, `endDate`, `url`, `location`) plus `tribe_events_cat-<slug>` classes on the surrounding markup for category filtering. Page 1 → 12 events, page 2 → 12, page 3 → 1, page 4 → 0 (clean stop condition), reaching all 25 published events. Of those, ~10 are public-facing (What's Up Downtown ×4, New Faces New Places ×3, New Member Coffee ×2, DMI Annual Celebration) and ~15 are internal governance meetings that should be filtered out via a category allowlist. Retired rather than rewritten because that's a new transport plus an allowlist for ~10 events over five months; the coverage is genuinely unique (zero overlap with Isthmus / Visit Madison), so revisit if DMI's public programming grows or the query-string redirect is fixed upstream.

### Alliant Energy Center
- URL: https://www.alliantenergycenter.com/upcoming-events
- Scraper type: html
- Status: **integrated**
- Notes: Integrated for #155 in May 2026. South-Madison expo / concert / sports complex (Veterans Memorial Coliseum, Exhibition Hall, New Holland Pavilions, Willow Island). The hypothesis from the issue holds up — large concerts route through Ticketmaster (covered by the TM scraper) while non-ticketed expos, agricultural shows, trade shows, and high school graduations do not. A spot-check of the live page (16 events, 2026-05-17 → 2026-08-02) found 13 of 16 events absent from every other integrated source: The Madison Classic Horse Show, Wisconsin State FFA Convention & Expo, Dane County Fair, Red Angus Junior National Show, Junior National Hereford Expo, North Central Reining Horse Association Futurity & Derby, Madison Sports Car Club Autocross, Craze Daze MADness, Big Gay PRIDE Market, Creating Keepsakes Scrapbook Convention, USTA Tumbling & Trampoline Championships, Swim Spa & Hot Tub Sale, Middleton/Cross Plains HS Graduation. DotNetNuke / Foremost Media CMS; single GET to `/upcoming-events` returns the full posted list as server-rendered HTML — no pagination, no JS, no API or iCal feed. `robots.txt` allows the path. Each `<li>` carries title, start/end dates (`M/D/YYYY` — no times, the page is dates-only), and a detail-page link. The detail page wraps the description in `div.nadevViewEventDetails.nadevViewEventDetailsPadding` blocks; the scraper picks the first non-empty block that isn't nested inside `[itemprop=offers]` (price scaffolding). Volume ~16 events / ~2.5 months; 1 s/page courtesy delay → ~16 s detail-enrichment overhead. Forward window honored locally (default 30 days). Titles ship entities raw (`Swim Spa &amp; Hot Tub Sale`), unescaped before hashing. Sub-rooms (Exhibition Hall, Willow Island, Veterans Memorial Coliseum) are dropped — the scraper emits `venue_name="Alliant Energy Center"` for every event, and the canonical-venue registry adds two aliases to merge cross-source listings: Isthmus's compound `Alliant Energy Center-Exhibition Hall` and Visit Madison's `Willow Island at Alliant Energy Center` both normalize to the building name before hashing. Verified coords (43.045136, -89.381338) and address (1919 Alliant Energy Center Way, Madison, WI 53713) come from the page's own `LocalBusiness` JSON-LD. **Ranked at the bottom of `SOURCE_PRIORITY`** (below City of Madison, the only other dates-only-ish source): because Alliant publishes no event times, placing it above any time-bearing source would let its all-day midnight `start_at`/`end_at` overwrite accurate times (e.g. Isthmus's bridal-expo 1–5 PM ticketing window, Visit Madison's Bubble Run 8–11 AM start). At the bottom of the stack Alliant operates as a pure null-filler on shared events — its multi-day end-date still fills Visit Madison's Brat Fest entry (Visit Madison ships `end_at=null`), and Alliant-only events insert as new rows. No source-category mapping: program tags are too specific (agricultural, expo) to fit our taxonomy; the LLM tagger handles them.

### 608today (6AM City)
- URL: https://608today.6amcity.com/events
- Scraper type: html → none
- Status: **rejected**
- Reason: Investigated for #73 in May 2026 and ruled out on three independent grounds. (1) **The live host is unreachable.** `608today.6amcity.com` is a `CNAME` to `sixam-city.production.k1.m1.brightspot.cloud`, which itself `CNAME`s to the CloudFront distribution `d16rso030wldb4.cloudfront.net`. That terminal name returns **no A or AAAA records** on either Google (`8.8.8.8`) or Cloudflare (`1.1.1.1`) DNS, and `WebFetch` against the same URL returns `ECONNREFUSED`. The `608today.com` apex still 301-redirects to the same unresolvable subdomain. Search and social results list the brand as live but the public CDN edge is gone — same failure pattern as the retired CloudFront/Imperva fronts that took Overture (#162) and Eventbrite (#182) off the table; we cannot validate any feed against it today. (2) **The feed shape is roundup, not per-event.** Wayback's 2024-05-22 snapshot of `/events.rss` (linked from `<link rel="alternate">` on `/events`) is a Brightspot CMS-generated RSS of **editorial articles**, not event records. Each `<item>` is a story like "Get jazzed for the Madison Jazz Festival" whose `<content:encoded>` is HTML embedding 10+ events as freeform `<b>Title</b> | Day | Time | Venue | Price | Description<br>` lines; `<pubDate>` is the article's publish date, not the event date. There is no per-event `start_at` / `end_at` / `venue_name`, so the implied integration path is per-article LLM extraction — a new ingest pattern not used by any current scraper and a real per-run token cost. (3) **Coverage overlap is high.** The sample roundup names venues we already cover via existing scrapers (High Noon Saloon, Memorial Union Terrace via Visit Madison, Arts + Literature Laboratory and MMoCA via Isthmus). The marginal unique-venue coverage 608today might add (Cafe CODA, Pinney Library, North Street Cabaret) is small relative to the integration cost, and Isthmus already surfaces several of those venues in its RSS. Revisit only if (a) the host comes back online on a stable, addressable origin, **and** (b) 6AM City publishes a per-event structured feed (JSON, ICS, or one item per event), **and** (c) a spot-check shows meaningful unique-venue coverage that Isthmus does not already carry.
